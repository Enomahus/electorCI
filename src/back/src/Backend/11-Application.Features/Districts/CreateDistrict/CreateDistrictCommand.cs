...
namespace Application.Features.Districts.CreateDistrict
{
    ...
    public class CreateDistrictCommandValidator : DistrictValidatorBase<CreateDistrictCommand>
    {
        public CreateDistrictCommandValidator(ReadOnlyDbContext context) : base(context)
        {
        }
    ...
    {
        public async Task<Result<long>> Handle(CreateDistrictCommand command, CancellationToken cancellationToken)
        {
        ...
        private async Task<string> GenerateDistrictCode(CreateDistrictCommand command, CancellationToken cancellationToken)
        {
            string parentIdPrefix = string.Empty;

            if(command.ParentId.HasValue)
            {
                var parent = await context.Districts
                    .AsNoTracking()
            ...
            var maxExistingCode = await context.Districts
                .Where(d => d.Level == command.Level && d.ParentId == command.ParentId)
                .Select(c => c.Code)
            ...
            if(!string.IsNullOrEmpty(maxExistingCode))
            {
                ...
                {
                    throw new InvalidOperationException($"Le code existant '{maxExistingCode}' a un format invalide.");
                }
            }
