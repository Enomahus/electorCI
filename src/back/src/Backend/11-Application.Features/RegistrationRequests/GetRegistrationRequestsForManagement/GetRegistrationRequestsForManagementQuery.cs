...
namespace Application.Features.RegistrationRequests.GetRegistrationRequestsForManagement
{
        ...
        >()
    {
        ...
        {
            //Remonter l'arbre pour trouver les IDs de la (ou des) r�gion(s) de l'utilisateur.
            var userRegionIds = await context
                ...
                .Where(id => id.HasValue) // S�curit� contre les orphelins
                .Select(id => id!.Value)
                .Distinct()
                .ToListAsync(token);

            // Fail-fast : Si l'utilisateur n'est rattach� � aucune r�gion, on stoppe l�.
            if (userRegionIds.Count == 0)
            {
                return [];
            }

            // �TAPE 2 : Descendre l'arbre pour r�cup�rer TOUS les sous-districts li�s � cette r�gion.
            // On r�cup�re uniquement les IDs (Select) pour �viter l'instanciation des entit�s EF en m�moire (Tracked Entities).
